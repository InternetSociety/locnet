...
- Set `JWT_SECRET` to a long random value before you start the application.
- Set `SESSION_COOKIE_SECURE=true` for a production HTTPS deployment.
...
- Database Admin (administrator session only): http://localhost:8000/admin/database/

Create the first administrator after the application database starts:

```bash
docker compose run --rm web-locnet python -m app.admin create-admin admin@example.com
```

The command reads the password from the terminal. It does not print or store the plain password.

Remove a user with the administration command:

```bash
docker compose run --rm web-locnet python -m app.admin remove-user user@example.com
```
...
The same directory is mounted read/write by the internal sqlite-web service. It is
available only through the Nginx-protected administrator editor at `/admin/database/`;
sqlite-web has no published port or separate credentials.
...
Install and run Playwright through Docker Compose:

```bash
docker compose run --rm --no-deps playwright npm ci
docker compose run --rm playwright npm test
```

The Playwright service starts an isolated application on port 8011. It uses disposable users, data, geospatial services, and map services.
